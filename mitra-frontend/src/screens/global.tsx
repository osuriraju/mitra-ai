...
export function CommandScreen() {
  ...
  const items = [
    ...(q ? [{ ic: 'plus', tone: 'accent', t: `Add task “${q}”`, sub: '', k: '↵', run: () => { const t = s.addTask({ title: q }); s.toast(`Task added · ${t.title}`); router.push(href('tasks-inbox')); } }, { ic: 'sparkles', tone: 'accent', t: ai?.proposal ? `${ai.proposal.title}` : `Ask Mitra: “${q}”`, sub: ai?.proposal ? `${ai.proposal.sub} · confirm in chat` : 'Natural language capture or question', k: '⇧↵', run: async () => { s.pushAiMessage({ who: 'user', text: q }); router.push(href('ai')); try { const rr = s.aiEnabled ? await s.askAi(q) : ask(q, useStore.getState()); s.pushAiMessage({ who: 'ai', text: rr.text, chart: rr.chart, preview: rr.proposal ? { kind: rr.proposal.kind, title: rr.proposal.title, sub: rr.proposal.sub, payload: rr.proposal } : undefined }); } catch (e) { s.pushAiMessage({ who: 'ai', text: `Sorry — ${errMsg(e)}` }); } } }] : []),
...
export function QuickAddScreen() {
  ...
  /** Enter: ask the real assistant; small captures auto-save (per Settings → AI), anything else is confirmed in chat. Falls back to the local parser if AI is off/unreachable. */
  const save = async () => {
    if (!text.trim() || busy) return; setBusy(true);
    let reply = r; if (s.aiEnabled) { try { reply = await s.askAi(text.trim()); } catch (e) { s.toast(errMsg(e), { tone: 'danger' }); } }
    setBusy(false);
    if (!reply?.proposal) { if (reply?.text && s.aiEnabled) { s.pushAiMessage({ who: 'user', text }); s.pushAiMessage({ who: 'ai', text: reply.text, chart: reply.chart }); router.push(href('ai')); return; } const t = s.addTask({ title: text.trim() }); s.toast(`Added to inbox · ${t.title}`); router.push(href('tasks-inbox')); return; }
    const p = reply.proposal;
    if ((p.kind === 'expense' && s.settings.autoSave === 'Never') || p.kind === 'plan' || p.kind === 'reschedule' || (p.kind === 'expense' && s.settings.autoSave === 'Small only' && p.amount > 500)) { s.pushAiMessage({ who: 'user', text }); s.pushAiMessage({ who: 'ai', text: reply.text, preview: { kind: p.kind, title: p.title, sub: p.sub, payload: p } }); router.push(href('ai')); return; }
    applyProposal(p, s); s.toast(`${p.kind === 'expense' ? 'Expense' : 'Task'} saved · ${p.title}`); router.push(p.kind === 'expense' ? href('money') : href('tasks-today'));
  };
  return (
    <TodayScreen overlay={
      <Sheet title="Quick add" back="today">
        <div className="composer"><Icon name="sparkles" /><input className="ph-in" autoFocus value={text} onChange={(e) => setText(e.target.value)} placeholder="Spent ₹450 on dinner · Call John tomorrow 6 pm · Ran 5K" onKeyDown={(e) => { if (e.key === 'Enter') save(); }} /><Btn ic="mic" kind="ghost" cls="sm" onClick={() => s.toast('Voice input needs the backend', { tone: 'accent' })} /><Btn ic={busy ? 'refresh' : 'arrow-right'} kind="primary" cls={`sm ${busy ? 'spin' : ''}`} onClick={save} /></div>
