...
export function TxForm({ open, onClose, editId, asSheet = false }: { open: boolean; onClose: () => void; editId?: string; asSheet?: boolean }) {
  ...
  const body = (
    <>
      ...
      <div className="split">
        <div className="field"><div className="lbl">{kind === 'transfer' ? 'From account' : 'Account'}</div><div className="input" style={{ cursor: 'pointer' }} onClick={() => setShowAcc((v) => !v)}><Icon name="wallet" /><span>{s.accounts.find((x) => x.id === acc)?.name} {s.accounts.find((x) => x.id === acc)?.mask}</span><span className="sfx"><Icon name="chevron-down" /></span></div><div className="hint">{s.accounts.length} accounts</div></div>
      ...
      {showAcc && <div className="card pad-0 keep" style={{ marginTop: -6 }}><List>{s.accounts.map((a) => <div key={a.id} className="setrow" style={{ minHeight: 44, padding: '8px 12px' }} onClick={() => { setAcc(a.id); setShowAcc(false); }}><IconBox ic={a.kind === 'credit' ? 'columns' : 'wallet'} tone={a.id === acc ? 'accent' : ''} cls="sm" /><div className="grow"><div className="t" style={{ fontSize: 13.5 }}>{a.name} <span className="muted" style={{ fontWeight: 500 }}>{a.mask}</span></div></div><span className="sm num muted">{a.balance < 0 ? '−' : ''}{inr(a.balance)}</span>{a.id === acc && <Icon name="check" cls="accent" />}</div>)}<div className="setrow" style={{ minHeight: 40, padding: '6px 12px' }}><Link className="link sm" href={href('profile')}>+ Add account</Link></div></List></div>}
...
function MoneySummary({ month, tx }: { month: string; tx: Transaction[] }) {
  ...
  return (<>
    <Card><div className="kpi-3"><Kpi l="Spent" v={inr(sp)} d={budget ? `of ${inr(budget)} budget` : ''} /><Kpi l="Income" v={inr(inc)} d={inc ? `${tx.filter((t) => t.kind === 'income').length} credit${tx.filter((t) => t.kind === 'income').length > 1 ? 's' : ''}` : 'none yet'} /><Kpi l="Saved" v={inr(tr)} d={inc ? `${Math.round((tr / inc) * 100)}% of income` : ''} dir={tr > 0 ? 'up' : ''} /></div><div className="mt"><Bar pct={budget ? (sp / budget) * 100 : 0} tone={sp > budget ? 'danger' : sp > budget * .9 ? 'warning' : ''} /></div><div className="row between xs muted mt-s"><span>{budget ? Math.round((sp / budget) * 100) : 0}% of budget{cur ? ` · ${left} days left` : ` · ${monthTitle(month)}`}</span>{cur && <span>Projected month end: {inr(projected)}</span>}</div></Card>
...
function MoneyAnalytics({ month, tx }: { month: string; tx: Transaction[] }) {
  ...
  return (<>
    <div className="stat-row">
      ...
      {tile('Net savings', `${net < 0 ? '−' : ''}${inr(net)}`, tr ? `incl. ${inr(tr)} moved to savings` : 'Monthly retained surplus', 'var(--accent)')}
